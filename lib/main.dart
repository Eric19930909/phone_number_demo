...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Validate phone number'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  String _prefix = "+852";
  final HashSet<String> _cacheSet = HashSet();
  ...
  void _onConfirm() {
    ...
    if (text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("请输入电话号码")));
      return;
    }
    // 存储在缓存中
    bool result = _cacheSet.add("$_prefix$text");
    if (result) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("保存成功")));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("已存在相同号码")));
    }
  }
  ...
  Future<Map<String, dynamic>?> getData() async {
    try {
      Dio dio = Dio();
      dio.interceptors.add(LogInterceptor(requestBody: true, responseBody: true, logPrint: LogUtil.e));
